        ...
        <div class="main-content-wrap">
            <div class="flex items-center flex-wrap justify-between gap20 mb-27">
                ...
                <ul class="breadcrumbs flex items-center flex-wrap justify-start gap10">
                    ...
                    <li>
                        <div class="text-tiny">New Delivery Timeslot</div>
                    </li>
                </ul>
            </div>
            ...
            <!-- new-category -->
            <div class="wg-box">
                <form class="form-new-product form-style-1" action="{{ route('admin_delivery_timeslot_create') }}" method="POST">
                    @csrf
                    <fieldset class="name">
                        <div class="body-title">Time Range <span class="tf-color-1">*</span></div>
                        <input class="flex-grow me-3 @error('time_range') is-invalid @enderror" type="text" placeholder="Time Range" name="time_range" value="{{ old('time_range') }}" tabindex="0" value="" aria-required="true">
                        @error('time_range')
                    ...
                    <fieldset class="name">
                        ...
                        <input class="flex-grow me-3 @error('slug') is-invalid @enderror" type="text" placeholder="Delivery Timeslot Slug" name="slug" value="{{ old('slug') }}" tabindex="0" value="" aria-required="true" readonly>
                        @error('slug')
                            <span class="invalid-feedback fs-5" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
                    ...
                    <fieldset class="name">
                        ...
                        <input type="text" class="flex-grow me-3 @error('start') is-invalid @enderror" id="timePickerFlatpickr1" name="start" value="{{ old('start') }}" placeholder="Select Start Time">
                    ...
                    <fieldset class="name">
                        ...
                        <input type="text" class="flex-grow me-3 @error('end') is-invalid @enderror" id="timePickerFlatpickr2" name="end" value="{{ old('end') }}" placeholder="Select End Time">
                            ...
                            <select class="" name="delivery_method_id">
                                ...
                                @if($delivery_methods->isNotEmpty())
                                    @foreach ($delivery_methods as $delivery_method)
                                        <option value="{{ $delivery_method->id }}" {{ (old('delivery_method_id') == $delivery_method->id) ? 'selected' : '' }}>{{ $delivery_method->name }}</option>
                                    @endforeach
                                @endif
                            </select>
                        </div>

    ...
    <div class="main-content-wrap">
        ...
        <div class="wg-box">
            ...
            <div class="flex items-center justify-between gap10 flex-wrap">
                ...
                <a class="tf-button style-1 w208" href="{{ route('admin_product_create_page') }}"><i class="icon-plus"></i>Add New Product</a>
            </div>
                    ...
                    <table class="table table-striped table-bordered">
                        <thead>
                            <tr>
                                ...
                                <th class="text-center p-2">Sale Price</th>
                                ...
                                <th class="text-center p-2">View</th>
                                    ...
                                    <tr>
                                        ...
                                        <td class="pname d-flex justify-content-center p-5">
                                            <div class="image">
                                                @if (!empty($product->image))
                                                    <img src="{{ asset('uploads/product/thumbnails/' . $product->image) }}" alt="{{ $product->name }}" class="image">
                                                @else
                                                    <img src="{{ asset('assets/admin/images/unavailable.png') }}" alt="image not available" class="image">
                                                @endif
                                            </div>
                                            <div class="name d-flex align-items-center">
                                                <a href="{{ route('admin_product_edit_page', $product->slug) }}" class="body-title-2">{{ $product->name }}</a>
                                            </div>
                                        </td>
                                        <td class="text-center p-2">{{ $product->sku }}</td>
                                        <td class="text-center p-2">₹{{ $product->price }}</td>
                                        <td class="text-center p-2">₹{{ $product->actual_price }}</td>
                                        <td class="text-center p-2">{{ $product->category }}</td>
                                        <td class="text-center p-2">{{ $product->brand }}</td>
                                        <td class="text-center p-2">
                                            @if ($product->is_featured == 1)
                                                <i class="bi bi-check-circle-fill text-success me-2"></i>Yes
                                            @else
                                                <i class="bi bi-x-circle-fill text-danger me-2"></i>No
                                            @endif
                                        </td>
                                        <td class="text-center p-2">
                                        ...
                                        <td class="text-center p-2">
                                            <div class="list-icon-function d-flex justify-content-center align-items-center">
                                                <a href="{{ route('admin_product_view_page', $product->slug) }}" target="_blank">
                                                    <div class="item eye">
                                                        <i class="icon-eye"></i>
                                                    </div>
                                                </a>
                                        ...
                                        <td class="text-center p-2">
                                            <div class="list-icon-function d-flex justify-content-center align-items-center">
                                                <a href="{{ route('admin_product_edit_page', $product->slug) }}">
                                                    <div class="item edit">
                                                        <i class="icon-edit-3"></i>
                                                    </div>
                                                </a>
                                                <a href="javascript:void(0)" onclick="deleteProduct('{{ $product->slug }}', '{{ $product->name }}')">
...
<script>
    function deleteProduct(productSlug, productName) {
        if(confirm(`Do you really want to delete this product - ${productName}?`)) {
            var url = "{{ route('admin_product_destroy', 'slug') }}";

        ...
        <div class="main-content-wrap">
            ...
            <div class="wg-box">
                ...
                <div class="flex items-center justify-between gap10 flex-wrap">
                    ...
                    <a class="tf-button style-1 w208" href="{{ route('admin_category_create_page') }}"><i class="icon-plus"></i>Add New</a>
                </div>
                                        ...
                                        <tr>
                                            ...
                                            <td class="pname d-flex justify-content-center p-5">
                                                <div class="image">
                                                    @if (!empty($category->image))
                                                        <img src="{{ asset('uploads/category/thumbnails/' . $category->image) }}" alt="{{ $category->name }}" class="image">
                                                    @else
                                                        <img src="{{ asset('assets/admin/images/unavailable.png') }}" alt="image not available" class="image">
                                                    @endif
                                                </div>
                                                <div class="name d-flex align-items-center">
                                                    <a href="{{ route('admin_category_edit_page', $category->slug) }}" class="body-title-2">{{ $category->name }}</a>
                                                </div>
                                            </td>
                                            <td class="text-center p-2">{{ $category->slug }}</td>
                                            <td class="text-center p-2">
                                                <a href="#" target="_blank">
                                                    {{ count_products_by_category($category->id) }}
                                                    {{-- 1 --}}
                                                </a>
                                            </td>
                                            ...
                                            <td class="text-center p-2">
                                                <div class="list-icon-function d-flex justify-content-center">
                                                    <a href="{{ route('admin_category_edit_page', $category->slug) }}">
                                                        <div class="item edit">
                                                            <i class="icon-edit-3"></i>
                                                        </div>
                                                    </a>
                                                    <a href="javascript:void(0)" onclick="deleteCategory('{{ $category->slug }}', '{{ $category->name }}')">
...
@push('scripts')
    <script>
        function deleteCategory(categorySlug, categoryName) {
            if(confirm(`Do you really want to delete this category - ${categoryName}?`)) {
                var url = "{{ route('admin_category_destroy', 'slug') }}";
                ...
                $.ajax({
                    ...
                    error: function(jqXHR, exception) {
                        alert('Error occured while removing the category!');
                    },
                });
            }
        }
